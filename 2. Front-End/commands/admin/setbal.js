...
class SetBalanceCommand extends commando.Command
{
    constructor(client)
    {
        super(client,{
            ...
            group: 'admin',
            memberName: 'setbal',
            description: 'Sets the user balance'
        });
    }

    async run(message, args)
    {
        if(await essentials.getInfo("SETBALANCECOMMAND") == "1") return message.channel.send(essentials.getDefaultMessage("commanddisabled"));
       ...
       if(!mod)
        ...
        if(args == "usage") return message.channel.send(essentials.getUsageInfo("setbal"));
        ...
        currDB.all(q, [], (err, rows) => {
            ...
            if(rows.length < 1){
                ...
                secondQuery = `UPDATE currency SET amount=${parseInt(args.split(' ')[1])} WHERE id='${user.id}'`;
                currDB.all(secondQuery, [], (err, rows)=>{
                    if(err) {console.log(err); return message.channel.send(essentials.getDefaultMessage("error"));}
                });
            }         
            return message.channel.send(essentials.craftMessage("check", `Balance for <@${user.id}> was set to ${args.split(' ')[1]}`)); 
        });
    }   
